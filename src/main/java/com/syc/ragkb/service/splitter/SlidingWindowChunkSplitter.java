package com.syc.ragkb.service.splitter;

import com.syc.ragkb.service.loader.ParseResult;
...
public class SlidingWindowChunkSplitter  implements ChunkSplitter{
    ...
    public List<ChunkResult> split(ParseResult parseResult, ChunkConfig config) {
        ArrayList<ChunkResult> chunks = new ArrayList<>();
        int chunkIndex = 0;
    ...
    /*
    * 从position 向前找到好的断点，段落＞句号》逗号> 空格。
    * 最多回退100字符，找不到就直接断点
    * */

    private int findGoodBreakPoint(String text, int position, int overlap) {
        int searchRange = Math.min(100, position - overlap); // 搜索范围

        //优先级：段落换行> 句号> 逗号> 空格
        String[] breakChars = {"\n\n","\n","。","！","？",";",","," "};

        for (String breakChar : breakChars){
            int index = text.lastIndexOf(breakChar, position);
            if(index > position - searchRange && index > 0){
                return  index + breakChar.length();
            }
        }
        //找不到好的断点，直接断点
        return  position;
